%% Example Laplace 2-d in a trapezoid
%Solves the Poisson problem in a trapezoidal with
%mixed boundary conditions. The numerical solution is compared to a given
%exact solution.
%
%
% -------------------------------------------------------------------------
...
% Boundary Conditions
prbl.BC.neum_sides = [ 1, 3];
prbl.BC.neum_lim = [ 0, 0; 1, 1];
prbl.BC.neum_fun{1} = @(x)( 0*x(1,:)  );
prbl.BC.neum_fun{2} = @(x)( (pi*sin( x(1,:)*pi ).*sin( x(2,:)*pi ))  );

prbl.BC.dir_sides = [ 2, 4];
prbl.BC.dir_lim = [ 0, 0; 1, 1];
